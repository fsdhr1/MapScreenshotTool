...
public class MapScreenshotToolCGCS2000 {
    ...
    public static void downLoadImage(double latitude,double longitude,int zoom,String imageUrl,String outPath,String outPathmsg){
        long y = MercatorProjectionCGCS2000.latitudeToTileY(latitude, (byte) zoom);
        long x = MercatorProjectionCGCS2000.longitudeToTileX(longitude, (byte) zoom);
        y = y-4;
        x = x-4;
        ...
        Envelope envelope = TileUtilCGCS2000.xyzs2Envelope2Pixel(lastTiles);
        ...
        String msg = "<PAMDataset>\n" +
                ...
                "  </Metadata>\n" +
                "</PAMDataset>\n";
        double xx = MercatorProjectionCGCS2000.tileXToLongitude(x,(byte) zoom);
        double yy = MercatorProjectionCGCS2000.tileYToLatitude(y,(byte) zoom);

        double xxe = MercatorProjectionCGCS2000.tileXToLongitude(x+8,(byte) zoom);
        double yye = MercatorProjectionCGCS2000.tileYToLatitude(y+8,(byte) zoom);
    ...
    public static void downLoadImage(double latitude,double longitude,Double[] bounds,int zoom,String imageUrl,String outPath,String outPathmsg){
        ...
        List<long[]> lastTiles = new ArrayList<>();
        long miny = MercatorProjectionCGCS2000.latitudeToTileY(bounds[0], (byte) zoom);
        long maxy = MercatorProjectionCGCS2000.latitudeToTileY(bounds[2], (byte) zoom);
        long maxx = MercatorProjectionCGCS2000.longitudeToTileX(bounds[1], (byte) zoom);
        long minx = MercatorProjectionCGCS2000.longitudeToTileX(bounds[3], (byte) zoom);
        ...
        String msg = "<PAMDataset>\n" +
                ...
                "  </Metadata>\n" +
                "</PAMDataset>\n";
        double xx = MercatorProjectionCGCS2000.tileXToLongitude(minx,(byte) zoom);
        double yy = MercatorProjectionCGCS2000.tileYToLatitude(miny,(byte) zoom);

        double xxe = MercatorProjectionCGCS2000.tileXToLongitude(maxx,(byte) zoom);
        double yye = MercatorProjectionCGCS2000.tileYToLatitude(maxy,(byte) zoom);
    ...
    public static BufferedImage createMapScreenshot(String wkt, List<String> waters, final String imgCustomUrl) {
        try {
            ...
            //Envelope envelope =TileUtilCGCS2000.bound2PixelEnvelope(boundingBox,16);
            ...
            final Envelope envelope = TileUtilCGCS2000.xyzs2Envelope2Pixel(tiles);
            ...
            for (final long[] tile : tiles) {
                FutureTask<String> futureTask = new FutureTask<String>(new Callable<String>() {
                    ...
                    public String call() throws Exception {
                        ...
                        Envelope envelopet = TileUtilCGCS2000.xyz2Envelope2Pixel(tile);
            ...
            double pminx = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.west(), (byte) zoom) - 100 - envelope.getMinX();
            double pmaxX = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.east(), (byte) zoom) + 100 - envelope.getMinX();
            double pminY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.north(), (byte) zoom) - 100 - envelope.getMinY();
            double pmaxY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.south(), (byte) zoom) + 100 - envelope.getMinY();
            ...
            for (int i = 0; i < geometries.length; i++) {
                ...
                if (geometries1 instanceof Polygon) {
                    cs = ((Polygon) geometries1).coordinates();
                }
                if (geometries1 instanceof MultiPolygon) {
                    List<List<List<Point>>> csss = ((MultiPolygon) geometries1).coordinates();
                ...
                for (List<Point> points : cs) {
                    ...
                    for (int j = 0, len = points.size(); j < len; j++) {
                        ...
                        xPoints[j] = (int) ((MercatorProjectionCGCS2000.longitudeToPixelX(c.longitude(), zoom)) - envelope.getMinX());
                        yPoints[j] = (int) ((MercatorProjectionCGCS2000.latitudeToPixelY(c.latitude(), zoom)) - envelope.getMinY());
                    }
                    //绘制标注需要做避让
                    ...
                    for (int j = 0, len = _convexHullcs.length; j < len; j++) {
                        ...
                        double pixelx = MercatorProjectionCGCS2000.longitudeToPixelX(c.x, zoom);
                        double pixely = MercatorProjectionCGCS2000.latitudeToPixelY(c.y, zoom);
                        if (anchor.equals("top-right") || anchor.equals("bottom-right")) {
                            ...
                            Envelope e = new Envelope(MercatorProjectionCGCS2000.pixelXToLongitude(xp1, zoom), c.x, MercatorProjectionCGCS2000.pixelYToLatitude(yp1, zoom), c.y);
                            boolean isIn = false;
                        ...
                        if (anchor.equals("top-left") || anchor.equals("bottom-left")) {
                            double xp1 = pixelx + graphics2D.getFontMetrics().charsWidth((c.x + "").toCharArray(), 0, (c.x + "").length());
                            double yp1 = pixely + graphics2D.getFontMetrics().getHeight() * 2;
                            Envelope e = new Envelope(c.x, MercatorProjectionCGCS2000.pixelXToLongitude(xp1, zoom), c.y, MercatorProjectionCGCS2000.pixelYToLatitude(yp1, zoom));
                            boolean isIn = false;
                            for (Envelope envelope1 : envelopes) {
                                if (envelope1.intersects(e)) {
                                    isIn = true;
                ...
                FutureTask<String> futureTask = new FutureTask<String>(new Callable<String>() {
                    ...
                    public String call() throws Exception {
                        ...
                        if (imgCustomUrl2 != null && !"".equals(imgCustomUrl2)) {
                            ...
                            envelopet = TileUtilCGCS2000.xyz2Envelope2Pixel(tile);
    ...
    public static  List<Object> createMapScreenshots(FeatureCollection featureCollection, final String imgCustomUrl, ScreenshotsSetting screenshotsSetting) {
        try {
            ...
            //List<long[]> tiles = TileUtilCGCS2000.getBoundTileV2(boundingBox.west(), boundingBox.south(), boundingBox.east(), boundingBox.north(), 15);
            //计算瓦片
            List<long[]> tiles = searchZoomTilesByAreaV2(screenshotsSetting,arr, boundingBox,pixelList);
            Envelope envelope = TileUtilCGCS2000.xyzs2Envelope2Pixel(tiles);
            int w = (int) (envelope.getMaxX() - envelope.getMinX());
            int h = (int) (envelope.getMaxY() - envelope.getMinY());
            BufferedImage finalImg = new BufferedImage(w, h, 1);
        ...
        try {
            ...
            for (int i = 0; i < geometries.length; i++) {
                ...
                for (int j = 0, len = cs.length; j < len; j++) {
                    c = cs[j];
                    xPoints[j] = (int) ((MercatorProjectionCGCS2000.longitudeToPixelX(c.x, zoom)) - envelope.getMinX());
                    yPoints[j] = (int) ((MercatorProjectionCGCS2000.latitudeToPixelY(c.y, zoom)) - envelope.getMinY());
                }
    ...
    public static List<long[]> searchZoomTiles(int[] arr, BoundingBox boundingBox, int key) {
        ...
        for (int i = arr.length - 1; i >= 0; i--) {
            int zoom = arr[i];
            List<long[]> tiles = TileUtilCGCS2000.getBoundTileV2(boundingBox.west(), boundingBox.south(), boundingBox.east(), boundingBox.north(), zoom);
    ...
    private static List<long[]>searchZoomTilesByAreaV2(ScreenshotsSetting screenshotsSetting,int[] arr,BoundingBox boundingBox,List<Double> pixelList){
        ...
        for (int i = arr.length - 1; i >= 0; i--) {
            long zoom = arr[i];
            b = (byte) zoom;
            double pminx = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.west(), b)-margin;
            double pmaxX = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.east(), b)+margin;
            double pminY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.north(), b)-margin;
            double pmaxY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.south(), b)+margin;
            ...
            if(xwidth < width && yheight < height ){
                ...
                lastTiles = TileUtilCGCS2000.getBoundTileV3(pminx-buffx, pmaxX+buffx, pminY-buffy, pmaxY+buffy, zoom);
    ...
    public static List<long[]> searchZoomTilesByArea(int[] arr, BoundingBox boundingBox, double key) {
        ...
        for (int i = arr.length - 1; i >= 0; i--) {
            ...
            Envelope envelope = TileUtilCGCS2000.xyzs2Envelope2Pixel(tiles);
            byte b = (byte) zoom;
            double pminx = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.west(), b);
            double pmaxX = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.east(), b);
            double pminY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.north(), b);
            double pmaxY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.south(), b);
...
//       BufferedImage finalImg = MapScreenshotTool.createMapScreenshot("POLYGON ((115.3021241 35.9554462, 115.3025061 35.9553684, 115.3025086 35.9553678, 115.3034777 35.9582167, 115.3031039 35.9583201, 115.3021241 35.9554462), (115.3026701 35.9565879, 115.3027016 35.9562056, 115.3030794 35.9573015, 115.3027226 35.9569872, 115.3026701 35.9565879))",l);
            ...
            FutureTask<String> futureTask = new FutureTask<String>(new Callable<String>() {
                ...
                public String call() throws Exception {
                    ...
                    if (imgCustomUrl != null && !"".equals(imgCustomUrl)) {
                        ...
                        imgUrl = MapScreenshotToolCGCS2000.MAPTYPE.DAY_MAP.getUrl().replace("{x}", String.valueOf(tile[1])).replace("{y}", String.valueOf(tile[2])).replace("{z}", String.valueOf(tile[0]));
                    }
            ...
            for (List<Point> points : cs) {
                ...
                if (centerSymbol != null && !centerSymbol.equals("")) {
                    ...
                    if (geoJson instanceof Point) {
                        ...
                        int x = (int) ((MercatorProjectionCGCS2000.longitudeToPixelX(point.longitude(), zoom)) - envelope.getMinX()) - width / 2;
                        int y = (int) ((MercatorProjectionCGCS2000.latitudeToPixelY(point.latitude(), zoom)) - envelope.getMinY()) + height / 2;
    ...
    private static int drawRectAndSzzb(BoundingBox boundingBox, Graphics2D graphics2D, byte zoom, Envelope envelope) {
        ...
        int rectX1 = (int) ((MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.west(), zoom)) - envelope.getMinX());
        int rectX2 = (int) ((MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.east(), zoom)) - envelope.getMinX());
        int rectY1 = (int) ((MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.north(), zoom)) - envelope.getMinY());
        int rectY2 = (int) ((MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.south(), zoom)) - envelope.getMinY());
        graphics2D.setColor(new Color(255, 255, 255));
        //绘制左上
    ...
    private static List<Object> cutImageWithPolygonCenterCustom(
        ...
        gr.drawImage(finalImg, 0, 0,
                ...
                new Double(pmaxX).intValue(), new Double(pmaxY).intValue(),
                null);
        BoundingBox boundingBox1 = BoundingBox.fromLngLats(MercatorProjectionCGCS2000.pixelXToLongitude(pminx+envelope.getMinX(), zoom),
                MercatorProjectionCGCS2000.pixelYToLatitude(pmaxY+envelope.getMinY(), zoom)
                , MercatorProjectionCGCS2000.pixelXToLongitude(pmaxX+envelope.getMinX(), zoom)
                , MercatorProjectionCGCS2000.pixelYToLatitude(pminY+envelope.getMinY(), zoom));
    ...
    private static List<Object> cutImageWithPolygonCenter(BoundingBox boundingBox, byte zoom, Envelope envelope, BufferedImage finalImg, int padding) {
        double pminx = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.west(), zoom) - (padding < 100 ? 100 : padding) - envelope.getMinX();
        if (pminx < 0) {
            pminx = 0;
        }
        double pmaxX = MercatorProjectionCGCS2000.longitudeToPixelX(boundingBox.east(), zoom) + (padding < 100 ? 100 : padding) - envelope.getMinX();
        if (pmaxX > finalImg.getWidth()) {
            pmaxX = finalImg.getWidth();
        }
        double pminY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.north(), zoom) - (padding < 100 ? 100 : padding) - envelope.getMinY();
        if (pminY < 0) {
            pminY = 0;
        }
        double pmaxY = MercatorProjectionCGCS2000.latitudeToPixelY(boundingBox.south(), zoom) + (padding < 100 ? 100 : padding) - envelope.getMinY();
