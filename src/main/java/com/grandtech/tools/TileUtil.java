...
public class TileUtil {
    ...
    public static List<long[]> getBoundTileV3(double pminx, double pmaxX, double pminY, double pmaxY, long z) {
        byte b = (byte) z;
        long minX = MercatorProjection.pixelXToTileX(pminx,b);
        long maxX = MercatorProjection.pixelXToTileX(pmaxX,b);
        long minY = MercatorProjection.pixelYToTileY(pminY,b);
        long maxY = MercatorProjection.pixelYToTileY(pmaxY, b);
    ...
    public static Map<Long, Long[]> computeAllZTileRegion(long z, long x, long y, long nextZ) {
        if (nextZ < z) throw new RuntimeException("nextZ Must big than z");
        Map<Long, Long[]> tileInfo = new HashMap<>();
        tileInfo.put(z, new Long[]{x, x, y, y});
        if (z == nextZ) return tileInfo;
        long sx = x << 1;
        long sy = y << 1;
        long ex = (x << 1) + 1;
        long ey = (y << 1) + 1;
    ...
    public static long[] computeZTileRegion(long z, long x, long y, long nextZ) {
        if (nextZ < z) throw new RuntimeException("nextZ Must big than z");
        if (z == nextZ) return new long[]{x, x, y, y};
        long sx = x << 1;
        long sy = y << 1;
        long ex = (x << 1) + 1;
        long ey = (y << 1) + 1;
        ++z;
        if (z == nextZ) return new long[]{sx, ex, sy, ey};
        while (++z <= nextZ) {
            sx = sx << 1;
            sy = sy << 1;
            ex = (ex << 1) + 1;
